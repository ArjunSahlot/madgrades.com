...
import CourseName from "./course/CourseName";
...
class Explorer extends Component {
	...
	componentWillMount = () => {
		const { entityType, actions, page, sort, order, minCountAvg, minGpaTotal, filterParams } = this.props;
		...
		switch (entityType) {
			...
			case "instructor":
				...
				return (
					<Header as="h4">
						<Header.Content>
							<Link to={`/professors/${instructor.id}`}>{instructor.name}</Link>
						</Header.Content>
					</Header>
				);
			case "subject":
					...
					<Table.Row>
						...
						>
							...
							<Popup trigger={<Icon color="grey" name="question circle" />}>
								<Popup.Content>
									The average number of students per grade distribution entry. This is often equivalent to
									the average number of students per course section.
								</Popup.Content>
							</Popup>
						</Table.HeaderCell>
						<Table.HeaderCell
						...
						<Table.HeaderCell onClick={this.onSortChange("gpa")} sorted={sort === "gpa" ? orderFull : null}>
							Avg. GPA{" "}
							<Popup trigger={<Icon color="grey" name="question circle" />}>
